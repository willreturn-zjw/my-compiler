...
| 32 | backend | 整数与浮点混合运算及比较转换 | FAIL | 1 | AssertionError('缺少模式：^\\s*fcmp\\w*\\s+') |
...
| 41 | backend | 内层 break/continue 的最近循环目标 | PASS | 1 | 所有静态断言通过 |
| 42 | backend | 菱形 CFG 合流后的变量定义与使用 | PASS | 1 | 所有静态断言通过 |
| 43 | backend | 非法 SysY 语法的诊断与产物阻断 | PASS | 1 | 所有静态断言通过 |
| 44 | backend | 无 -S、-O 等级与指定输出路径 | PASS | 2 | 所有静态断言通过 |

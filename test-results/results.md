...
| 19 | backend | 有符号除法与取模 | PASS | 1 | 所有静态断言通过 |
| 20 | backend | 整数比较运算 | PASS | 1 | 所有静态断言通过 |
| 21 | backend | 浮点算术与比较 | PASS | 2 | 所有静态断言通过 |
| 22 | backend | 局部变量分配与读写 | PASS | 2 | 所有静态断言通过 |
| 23 | backend | 全局变量与全局数组 | PASS | 1 | 所有静态断言通过 |
| 24 | backend | 一维数组边界地址计算 | PASS | 1 | 所有静态断言通过 |
| 25 | backend | 多维数组地址计算 | PASS | 2 | 所有静态断言通过 |
| 26 | backend | 条件分支真假路径 | PASS | 2 | 所有静态断言通过 |
...
| 31 | backend | 逻辑与、逻辑或和逻辑非的短路控制流 | PASS | 2 | 所有静态断言通过 |
...
| 38 | backend | 大局部数组的首尾元素读写 | FAIL | 2 | AssertionError('末元素地址未按 1023×4 = 4092 字节偏移构造') |
...
| 43 | backend | 非法 SysY 语法的诊断与产物阻断 | FAIL | 5 | AssertionError('语法错误应正常非零退出，实际 -6（负数表示信号崩溃）') |
| 44 | backend | 无 -S、-O 等级与指定输出路径 | PASS | 3 | 所有静态断言通过 |
| 45 | backend | 含空格的输入和汇编输出文件名 | PASS | 1 | 所有静态断言通过 |
| 46 | backend | 同目录连续编译的 IR 覆盖与状态重置 | PASS | 3 | 所有静态断言通过 |

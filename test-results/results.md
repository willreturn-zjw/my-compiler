...
| 15 | frontend | 数组形参与 getptr/getelemptr | PASS | 1 | 所有静态断言通过 |
...
| 29 | backend | 浮点及混合参数传递 | PASS | 1 | 所有静态断言通过 |
| 30 | backend | 高寄存器压力与 spill/reload | PASS | 1 | 所有静态断言通过 |
| 31 | backend | 逻辑与、逻辑或和逻辑非的短路控制流 | PASS | 3 | 所有静态断言通过 |
| 32 | backend | 整数与浮点混合运算及比较转换 | PASS | 2 | 所有静态断言通过 |
| 33 | backend | void 函数调用与隐式返回 | PASS | 2 | 所有静态断言通过 |
| 34 | backend | 整数与浮点 I/O 及计时函数调用 | PASS | 2 | 所有静态断言通过 |
| 35 | backend | 递归函数的返回值、分支与链接寄存器保护 | PASS | 2 | 所有静态断言通过 |
| 36 | backend | 嵌套多函数调用与中间返回值保存 | PASS | 2 | 所有静态断言通过 |
| 37 | backend | 大局部数组的首尾元素读写 | PASS | 2 | 所有静态断言通过 |
| 38 | backend | 9 个整数与 9 个浮点混合参数溢出到栈 | PASS | 4 | 所有静态断言通过 |
| 39 | backend | 浮点函数返回值参与后续比较 | PASS | 2 | 所有静态断言通过 |
| 40 | backend | 内层 break/continue 的最近循环目标 | PASS | 2 | 所有静态断言通过 |
| 41 | backend | 菱形 CFG 合流后的变量定义与使用 | PASS | 2 | 所有静态断言通过 |
| 42 | backend | 非法 SysY 语法的诊断与产物阻断 | PASS | 1 | 所有静态断言通过 |
| 43 | backend | 无 -S、-O 等级与指定输出路径 | PASS | 3 | 所有静态断言通过 |
| 44 | backend | 含空格的输入和汇编输出文件名 | PASS | 1 | 所有静态断言通过 |
| 45 | backend | 同目录连续编译的 IR 覆盖与状态重置 | PASS | 3 | 所有静态断言通过 |
